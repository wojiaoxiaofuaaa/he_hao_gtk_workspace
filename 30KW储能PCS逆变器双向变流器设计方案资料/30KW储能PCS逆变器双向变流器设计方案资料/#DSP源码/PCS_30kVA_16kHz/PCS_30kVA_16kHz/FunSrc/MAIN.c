	...
	static INT16U uwDelay400ms = 0;
			    	    ...
			    	    {		        
			    	        ...
			    	        {										                  	
			    	            ...
			    	            else
			    	            {
			    	            	if(fIsoStatue == 0)
			    	            	{
			    	            		if((wEepromISOEnable == 1)&&(fBatteryInitiStateOK == 2)&&(fFactoryFastStart == 0))
			    	            		{
			    	            			wBatVolLimtLow = ((INT32S)wBatDisChargeVoltLimt * 26214)>>14;
			    	            			
			    	            			if((fBatteryReady == 1)&&(wBatteryOutAvgVol >= wBatVolLimtLow))
			    	            			{
        ...
        case FaultMode:
        {
            ...
            else
            {
                ...
                if(fIsoLowErrEvent||fGridRelayErrEvent||fRunInvAhdErrEvent||fDampResistanceErrEvent
                   ||fInvRelayErrEvent||fAuxiRelayErrEvent||fDCRelayErrEvent||fDCPWMErrEvent||fDCSoftRelayErrEvent)
                {
...
//====Main Loop ===//
	while(1)							// Dummy loop.  Wait for an interrupt.
    {
        ...
        else if(ft20ms == 1)
        {
            ...
            wIqRefBakToLVRT = sIqRefExtract(&tInvCurrDqoRef);
