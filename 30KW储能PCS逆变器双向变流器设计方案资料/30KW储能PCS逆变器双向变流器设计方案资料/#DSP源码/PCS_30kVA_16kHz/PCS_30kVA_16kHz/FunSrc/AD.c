...
INT16U uwHalfCysRateVolSqure;
INT32U udwHalfCysRateVolSqure0R81;
INT32U udwHalfCysRateVolSqure1R21;
INT32U udwHalfCysRateVolSqure0R2;
INT32U udwHalfCysRateVolSqure0R01;
...
#pragma DATA_SECTION(wGFCICurrRms,"RamSafetyProt"); 
#pragma DATA_SECTION(wGFCIAvgCurr,"RamSafetyProt"); 
...
#pragma DATA_SECTION(twInvPhaseRmsVol,"RamSafetyProt");
#pragma DATA_SECTION(twInvPhaseAvgVol,"RamSafetyProt");
#pragma DATA_SECTION(twGridLineRmsVol,"RamSafetyProt");
#pragma DATA_SECTION(wBatteryOutAvgVol,"RamSafetyProt");
#pragma DATA_SECTION(wBatteryInAvgVol,"RamSafetyProt");
#pragma DATA_SECTION(wBusAvgVol,"RamSafetyProt");
#pragma DATA_SECTION(fBiDirDCEnOut,"RamSafetyProt");
#pragma DATA_SECTION(fInvEnOut,"RamSafetyProt");
...
INT32U udwMaxTemp,udwMinTemp;
    if(fLVRTEnable == 1)
    {
        ...
        if((udwMinTemp > udwHalfCysRateVolSqure0R81)&&(udwMaxTemp < udwHalfCysRateVolSqure1R21))
        {
            fLVRTValue = 16384;
            fHVRTValue = 16384;
            fImbalanceLVRT = 0;
        }
        else if((udwMinTemp <= udwHalfCysRateVolSqure0R81)&&(wEepromLVRTOption == 1))//LVRT priotity
        {
            uddwMinTemp = (INT64U)udwMinTemp<<14;
            udwtemp = uddwMinTemp/uwHalfCysRateVolSqure;//Q28
            fLVRTValue = sqrt(udwtemp);//Q14
            fHVRTValue = 16384;
            if((fLVRTStartOrEnd == 0)||(fImbalanceLVRT == 1))
            {
                udwtemp = udwHalfCysRateVolSqure0R01 + (((INT64U)udwHalfCysRateVolSqure0R2 * fLVRTValue)>>14);//(fLVRTValue+0.1)^2 - fLVRTValue^2  Q15
        ...
        else if((udwMaxTemp >= udwHalfCysRateVolSqure1R21)&&(wEepromHVRTOption == 1))
        {
        	uddwMaxTemp = (INT64U)udwMaxTemp<<14; 
        	udwtemp = uddwMaxTemp/uwHalfCysRateVolSqure;//Q28
        	fHVRTValue = sqrt(udwtemp);//Q14 
            fLVRTValue = 16384;
            if((fLVRTStartOrEnd == 0)||(fImbalanceLVRT == 1))
            {
                udwtemp = (((INT64U)udwHalfCysRateVolSqure0R2 * fHVRTValue)>>14) - udwHalfCysRateVolSqure0R01;//fHVRTValue^2 - (fHVRTValue-0.1)^2  Q15
	...
	*pVoltMulCurrSumFlag = 1;   
    ...
    for(j=0;j<4;j++)
    {
        if(uwPowerSumCnt != 0) 
        {
            if(j ==3 )
            {
            	dwCapZ = ((INT32S)915 * wZAtmp)>>12;//3Uab * 3Uab = 27Ua^2, 2*pi*50*300/1000000/27*3 Q20 PCS = 25uF  915
                i = (ddwPowerSum[j] / uwPowerSumCnt) + ((((INT64S)wGridRmsVolSum * wGridRmsVolSum)* dwCapZ)>>24);
            }
            else
            {
            	dwCapZ = ((INT32S)305 * wZAtmp)>>12;//305
                i = (ddwPowerSum[j] / uwPowerSumCnt) + ((((INT64S)wGridRmsVolSum * wGridRmsVolSum) * dwCapZ)>>24);
            }
        }
        else 
...
INT32S wPTmp,wQTmp;    
    ...
    for(j=0;j<4;j++)
    {
    	...
    	if(wPTmp < 0)
    	{
		    *(pInvReactivePower+j) = (((INT64S)wQTmp * 32767) + ((INT64S)wPTmp * 143))>>15; //实验校准系数0.25度
            *(pInvActivePower+j) = (((INT64S)wPTmp * 32767) - ((INT64S)wQTmp * 143))>>15; //实验校准系数0.25度
        }
        else
        {
        	*(pInvReactivePower+j) = (((INT64S)wQTmp * 32767) - ((INT64S)wPTmp * 143))>>15; //实验校准系数0.25度
