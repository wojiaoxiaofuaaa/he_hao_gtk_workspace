        ...
        if(((wEerpomOvVWEnable == 0)||(fVoltOvStartOrEnd == 0))&&((wEerpomUnVWEnable == 0)||(fVoltUnStartOrEnd == 0)))
        {
            if((fLVRTValue > uwLVRTActiveValue)&&(fHVRTValue < uwHVRTActiveValue))
            {
                if((wGridCrossZeroFreqMax > wOVFreTripFre)&&(wEepromOvFWEn == 1))
                {
					...
					if(wRemoteCSPPer<0||wEepromCSPPer<0)wTemp = wPOVFrefrozen + wTemp;
					else wTemp = wPOVFrefrozen - wTemp;
                    ...
                    else if(wTemp < -667)
                    {
                     ...
                     if(wRemoteCSPPer<0||wEepromCSPPer<0)
                	{
						if(wTemp < 0)
						{
						...
						if(wActivePowerPer>=0)wActivePowerPer=0;
                	}
					else
					{
					   if(wTemp >0)
    	...
    	case 3:
            ...
            else
            {
            	if((fBiDirDCEnOut >= 1)&&(fError == 0))
		        {
		            ...
		            {
						if(++uwDelay2>=2000)fDCPWMLostPreChk=1;
						uwDelay1 = 0;
		            }
                }
    		}  		
